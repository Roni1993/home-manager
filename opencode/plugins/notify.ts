...
// Returns true if a child session (e.g. a background delegation) is still running.
// The parent can be idle while delegated agents keep working, so notifications
// must wait until the whole session tree has settled.
async function hasRunningChildSessions(client: OpencodeClient, sessionID: string): Promise<boolean> {
	try {
		const childrenResult = await client.session.children({ path: { id: sessionID } })
		...
		// If we can't verify, assume work is still running to avoid premature notifications
		return true
	}
}
...
async function handleSessionIdle(
	...
	// Suppress while background child sessions (delegations) are still working
	if (await hasRunningChildSessions(client, sessionID)) return
