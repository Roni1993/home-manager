{ pkgs, lib, inputs, ... }: {
  ...
  home.file.".config/matugen/config.toml" = {
    text = ''
      ...
      # ';' not '&&': matugen runs post_hooks via $SHELL, which is nushell,
      # and nu rejects POSIX '&&'
  ...
  # Survive gamescope-session starts: bouncing graphical-session.target can
  # restart waybar/swaync before WAYLAND_DISPLAY points at a real socket
  # (Sep 2026: both crash-looped 5x in ~1s → start-limit-hit → dead bar +
  # dead notifications until manual reset-failed). Slow the restarts and
  # raise the limit so they ride out the race instead of giving up.
  ...
  home.file.".local/bin/apply-theme.sh" = {
    ...
    text = ''
      ...
      # timeout: if swaync is dead/wedged the client blocks forever (seen:
      # 4-day hangs), which silently skips the kitty reload below
