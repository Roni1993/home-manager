...
- [ ] Test Steam Game Mode session in plasmalogin/SDDM — #18; `gamescope-session.desktop` present, enter via HM-managed `~/.local/bin/switch-session` (nix, gaming.nix; calls root helper `fleek-set-session` via scoped sudoers; `steam-set-session` is a no-op on this plasmalogin build); quitting Steam in game mode auto-restores Hyprland autologin via the `gamescope-session.service.d/10-restore-desktop.conf` drop-in (ExecStopPost → `fleek-set-session hyprland --no-restart`) — autologin now points at Hyprland before the DM restarts, so no Game Mode loop — needs one real round-trip to mark done (round trip now works; Steam launches; see gamescope-session rows below for the still-open bits)
- [x] Debug gamescope Game Mode: **HDR advertisement was the flicker** — Steam-side HDR toggles hard-glitched the LG link (4K@60 HDR on a 2.0-class HDMI = bandwidth-starved chroma). Fixed by stripping `STEAM_GAMESCOPE_HDR_SUPPORTED` before Steam starts: `gamescope-session.service.d/20-no-hdr.conf` (drop-in, sed over the CachyOS wrapper, nix gaming.nix). Verified: flicker gone. Related gotchas known from Aug-29 debugging: stale `WAYLAND_DISPLAY` must be unset (10-restore-desktop.conf) or gamescope aborts; `hyprland-uwsm.desktop` had exited-127 the greeter (removed in bootstrap.sh); `DRM_MODE=cvt` drop-in was NOT needed.
- [x] **DECISION: scrap Steam Game Mode (gamescope-session) — 2026-09-08** — after flicker (HDR, fixed) the session still had unresolvable input delay + fps capped at the 60Hz sink; Steam FPS-limiter UI absent on desktop; `GAMESCOPE_ALLOW_TEARING=1` + async-flips removal didn't cure it. Switching sessions was fragile (stale `WAYLAND_DISPLAY`, greeter state). Chosen route: **Big Picture run from the Hyprland desktop** (Steam launched normally, set to Big Picture on the LG). gamescope drop-in `20-no-hdr` removed; `10-restore-desktop.conf` + `switch-session` kept as safety net (loop prevention, generic session flips).
